...
namespace Collabco.SussexDowns.WebParts.MyCourseAttendance
{
    [ToolboxItemAttribute(false)]
    public class MyCourseAttendance : Microsoft.SharePoint.WebPartPages.WebPart
    {
        public const string TILE_NAME = "cattendance";
        ...
        private int _exampleSetting = 50;
        //[WebBrowsable(true),
        //Personalizable(PersonalizationScope.Shared, false),
        //Category("Example Settings"),
        //WebDisplayName("Example Setting 1"),
        //WebDescription(@"An example setting.")]
        //public int ExampleSetting
        //{
        //    get { return _exampleSetting; }
        //    set { _exampleSetting = value; }
        //}
        ...
        {
            this.PreRender += MyAttendance_PreRender;
        }

        void MyAttendance_PreRender(object sender, EventArgs e)
        {
            using (SPMonitoredScope monitoredScope = new SPMonitoredScope("Mars " + this.Title + " Pre-Render (" + _IDENTITY + ")"))
            {
                ...
                if (!this.Page.ClientScript.IsClientScriptBlockRegistered("mars-" + TILE_NAME + "-trigger-" + _IDENTITY))
                {
                    ...
                    s.AppendLine(string.Format("    setTimeout(function(){{spinCourseAttendanceTile(\"{0}\", {1})}},Math.floor((Math.random()*2500)+1));", _IDENTITY, _exampleSetting)); //Example call to javascript function to render tile data passing example setting 1 through.
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            using (SPMonitoredScope monitoredScope = new SPMonitoredScope("Mars " + this.Title + " Render"))
            {
                ...
                _modalContent.Append(Util.GenerateIconHeading(null, "Month", "month-" + _IDENTITY, false, "month-head-" + _IDENTITY));
