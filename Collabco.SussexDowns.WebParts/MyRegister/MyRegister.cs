...
namespace Collabco.SussexDowns.WebParts.MyRegister
{
    [ToolboxItemAttribute(false)]
    public class MyRegister : WebPart
    {
        public const string TILE_NAME = "register";
        ...
        WebDisplayName("Link URL"),
        WebDescription(@"Put the URL here that should be open on click of the tile")]
        ...
        {
            this.PreRender += MyRegister_PreRender;
        }

        void MyRegister_PreRender(object sender, EventArgs e)
        {
            using (SPMonitoredScope monitoredScope = new SPMonitoredScope("Mars " + this.Title + " Pre-Render (" + _IDENTITY + ")"))
            {
                ...
                if (!this.Page.ClientScript.IsClientScriptBlockRegistered("mars-" + TILE_NAME + "-trigger-" + _IDENTITY))
                {
                    ...
                    s.AppendLine(string.Format("    setTimeout(function(){{spinRegisterTile(\"{0}\", \"{1}\")}},Math.floor((Math.random()*2500)+1));", _IDENTITY, _linkURL)); //Example call to javascript function to render tile data passing example setting 1 through.
