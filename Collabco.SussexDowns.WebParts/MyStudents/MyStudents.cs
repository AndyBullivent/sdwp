...
namespace Collabco.SussexDowns.WebParts.MyStudents
{
    [ToolboxItemAttribute(false)]
    public class MyStudents : WebPart
    {
        public const string TILE_NAME = "students";
        ...
        {
            this.PreRender += MyStudents_PreRender;
        }

        void MyStudents_PreRender(object sender, EventArgs e)
        {
            using (SPMonitoredScope monitoredScope = new SPMonitoredScope("Mars " + this.Title + " Pre-Render (" + _IDENTITY + ")"))
            {
                ...
                if (!this.Page.ClientScript.IsClientScriptBlockRegistered("mars-" + TILE_NAME + "-trigger-" + _IDENTITY))
                {
                    ...
                    s.AppendLine(string.Format("    setTimeout(function(){{spinStudentsTile(\"{0}\", \"{1}\")}},Math.floor((Math.random()*2500)+1));", _IDENTITY, _linkURL)); //Example call to javascript function to render tile data passing example setting 1 through.
